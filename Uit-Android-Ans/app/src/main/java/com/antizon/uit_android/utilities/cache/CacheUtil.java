...
public final class CacheUtil {
     ...
     */
    public static void cache(DataSpec dataSpec, Cache cache, @Nullable CacheKeyFactory cacheKeyFactory,
                             DataSource upstream, @Nullable ProgressListener progressListener, @Nullable AtomicBoolean isCanceled)
            throws Exception {
        cache(
                dataSpec,
                cache,
    ...
    private static long readAndDiscard(
            ...
            throws IOException, InterruptedException {
        long positionOffset = absoluteStreamPosition - dataSpec.position;
        while (true) {
...
//                dataSpec = new DataSpec(dataSpec.uri, dataSpec.httpMethod, dataSpec.httpBody, absoluteStreamPosition,dataSpec.position + positionOffset, C.LENGTH_UNSET, dataSpec.key, dataSpec.flags)

                dataSpec = new DataSpec.Builder()
                        .setUri(dataSpec.uri)
                        .setHttpMethod(dataSpec.httpMethod)
                        .setHttpBody( dataSpec.httpBody)
                        .setUriPositionOffset(absoluteStreamPosition)
                        .setPosition(dataSpec.position + positionOffset)
