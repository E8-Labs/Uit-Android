...
public class CommentsAdapter extends RecyclerView.Adapter<CommentsAdapter.ViewHolder> implements CommentRepliesAdapter.CommentRepliesAdapterCallBack {
    ...
    CommentsAdapterCallBack callBack;

    CommentRepliesAdapter repliesAdapter;
    public CommentsAdapter(Context context, List<CommentDataModel> commentsList, CommentsAdapterCallBack callBack) {
    ...
    public ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (viewType == VIEW_TYPE_LOADING){
            ...
            v = LayoutInflater.from(context).inflate(R.layout.single_item_comments, parent, false);
        }
    ...
    public void onBindViewHolder(@NonNull ViewHolder holder, int position) {
        if (holder.getItemViewType() != VIEW_TYPE_LOADING) {
            ...
            if (comment !=null){
                ...
                if (comment.isAlreadyLoaded()){
                    showCommentsRepliesRecyclerview(holder.recyclerview_replies, comment.getRepliesList(), String.valueOf(comment.getId()));
                }

                holder.layout_replies.setOnClickListener(v -> {
                    if (comment.getComments() !=0){
                        if (!comment.isAlreadyLoaded()){
                            callBack.onCommentRepliesClicked(position);
                            comment.setAlreadyLoaded(true);
                            comment.setShown(true);
                        }else {
                            comment.setShown(!comment.isShown());
                            notifyDataSetChanged();
                        }
                    }
                });

                holder.text_replyComment.setOnClickListener(v -> callBack.onReplyToCommentClicked(position));
            }
        }
    }
    ...
    private void showCommentsRepliesRecyclerview(RecyclerView recyclerView, List<CommentDataModel> commentsList, String parentCommentId) {
        recyclerView.setHasFixedSize(true);
        LinearLayoutManager linearLayoutManager = new LinearLayoutManager(context, LinearLayoutManager.VERTICAL, false);
        recyclerView.setLayoutManager(linearLayoutManager);
        repliesAdapter = new CommentRepliesAdapter(context, commentsList ,parentCommentId,this);
    ...
    public static class ViewHolder extends RecyclerView.ViewHolder{
        ...
        public ViewHolder(@NonNull View itemView) {
            ...
            layout_replies = itemView.findViewById(R.id.layout_replies);
            recyclerview_replies = itemView.findViewById(R.id.recyclerview_replies);
        }
    }

    public interface CommentsAdapterCallBack{
        void onLikeClicked(String commentId);
        void onCommentRepliesClicked(int position);
        void onReplyToCommentClicked(int position);
    }
}
